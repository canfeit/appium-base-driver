...
let deviceid = "";
let genTool = false;
let reportPath = 'C:/Users/yxin/Desktop/Projs/files';
...
function getReportFileName(reportPath, reportListName) {
    return fileSystem.existsSync(reportPath+'/'+reportListName+'.json') ?
            reportFileName + lineCount(reportPath+'/'+reportListName+'.json') :
            reportFileName + 0;
}
...
testwa.initBaseDriver = function (driver) {
    ...
    fileSystem.appendFileSync(reportPath+'/'+reportListName+'.json', JSON.stringify(reportList)+endOfLine);
...
testwa.getActionAndroid = async function(driver,req,httpStatus,httpResBody,commond,jsonObj){
    ...
    if (genTool) {
        // console.log(testDataReply);
        logData.error(stringify(testDataReply));
        if (report) {generateReportSteps(testDataReply,tempPng);
...
testwa.getActionIOS = async function (driver, req, httpStatus, httpResBody, commond, jsonObj) {
    ...
    let [testDataReply,endTime] = testwa.genRsp(driver,req,httpStatus,httpResBody,action,param,0,0);
    let screenshotPath = args ? args.screenshotPath : "";
