...
testwa.parseBatteryStats = function (statsStr, appPackage) {
    let statsArray = statsStr.split(endOfLine);
    let uid = null;
    let totalPower = null;
    let appUsedPower = null;
...
testwa.getBatterStats = function (driver) {
    let adbPath = driver.battery.adbPath;
    execSync(`${adbPath} shell dumpsys battery reset`);
    let out = spawnSync(adbPath, ['shell', 'dumpsys', 'batterystats', '--charged', '--checkin']);
    let batteryStatsStr = out.stdout.toString('utf8');
    // console.log(out.stdout.toString('utf8'));
    driver.adbPath = null;
    return testwa.parseBatteryStats(batteryStatsStr, driver.battery.package);;
};
...
testwa.responseDeleteSession = async function (driver, req, httpStatus, httpResBody, commond, jsonObj) {
    ...
    if (driver.args.batteryStats && driver.battery) {
        [testDataReply.testdata.battery, testDataReply.testdata.network] = testwa.getBatterStats(driver);
    }
...
testwa.initBatteryStats = function (driver, osDriver) {
    driver.battery = {};
    driver.battery.adbPath = osDriver.adb.sdkRoot + '/platform-tools/adb';
    driver.battery.package = osDriver.opts.appPackage;
    ...
    if (command !== 'deleteSession' && command !== 'createSession') {
        ...
        if (osDriver) {
            ...
            if ('android' === platformName) {
                ...
                if (driver.args.batteryStats && command === 'createSession') {
                    testwa.initBatteryStats(driver, osDriver);
                }
        ...
        if ('android' === platformName && driver.args.batteryStats && command === 'createSession') {
            testwa.initBatteryStats(driver, osDriver);
        }
